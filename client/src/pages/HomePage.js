...
const HomePage = () => {
  ...
  const getTotal = async () => {
    try {
      const { data } = await axios.get("/api/v1/plant/plant-count");
      setTotal(data?.total);
    } catch (error) {
      console.log(error);
    }
  ...
  const loadMore = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`/api/v1/plant/plant-list/${page}`);
      setLoading(false);
